...
- `metal-api-emulator` 的 `shared-provider-objects` 分支（推送目标 `origin/main`）：HEAD `cc38dd4`（2026-09-11），已推送；其中 `69dab48` 为 metal2vulkan pin 升级，`76f6102` 为异步 completion/readback 契约，`31a0ba8` 为 Vulkan 真实异步 provider，`ebca75e` 为 native Metal completion handler 与共享 `CompletionRecord`，`15bf79a` 为 Vulkan 设备侧 fence 等待，`45a7427` 为共享可配置 observation deadline，`6e5df5b` 为 reims 适配器跨 translator 版本桥接，`81d6cbe` 把可选 reims 集成纳入 CI，`eed4a53` 为 Vulkan 超时提交的 retirement 回收，`909a5fb` 为显式取消契约与对象 API，`0630022` 为 native 超时观察后保持可用，`008fe7d` 把 README 与 provider 文档同步到新的超时/取消语义，`3bf3c04` 新增 v8 binary AIR encoding conformance 套件（raw/wrapped），`d8e42bf` 让 Swift native oracle 接受 v8，`042acc8` 把 v8 套件写入 README 与 provider/conformance 文档并扩展 object-path Python 测试，`a0b10e1` 增加有界放弃预算、`ProviderHealth` 与设备丢失销毁路径，`7e67a0e` 增加 native `MTLCommandBufferError::DeviceRemoved` 分类与对象 API submit 期 `DeviceLost`/`Exhausted` 错误矩阵，`8d2a4ca` 把 Windows RTX 5060 v1–v8 验证写入 README 与 provider 文档，`d26359c` 在 core 增加 completion 驱动的 `LeaseLedger`，`c610cab` 在 core 增加 transport 无关的跨进程 completion 镜像层，`9fd56eb` 增加发送端 `CompletionPublisher` 与 `LoopbackTransport`，`2324252` 新增 `metal-api-ipc` crate 把 completion 流落到任意 `Read`+`Write` 字节流上（版本化 `MCW1` 帧 codec、`CompletionTransport<R, W>`、Unix socket pair/listener 助手与真实子进程集成测试），`3d4dbd7` 增加 owner 侧 `CompletionReceiver`（transport→mirror→lease 退休），`9226ef2` 在 core 增加 `CompletionObserver` 与 `CompletionOutbox`，`a06f7dc` 让 Vulkan/native provider 经 epoch 校验的 outbox 发布 admission/终态/health（`95bedff` 修复 native health 发布的 state 锁重入），`83b3f23` 在 `metal-api-ipc` 增加后台 `CompletionSender` writer 线程，`8913e37` 让 provider-smoke 用真实 Vulkan async provider 经 outbox→IPC writer→Unix socket 到 owner `CompletionReceiver` 并退休 lease，`6500b94` 补记 `integration/reims/Cargo.lock` 的 `metal-api-ipc` 条目以修复 reims `--locked`，`9ac0dfe` 让 provider-smoke 以 `--completion-child` 在两进程间完成 provider→Unix socket→owner lease 退休，`ab97ac9` 让 Vulkan provider 支持 staged lease 导入（`StagedLease`/`LeaseRegistry`/`LeaseImporter`），`42974ac` 让 Vulkan provider 在 `VK_EXT_external_memory_host` 可用时支持 borrowed no-copy host mapping 导入（`BorrowedLease`/`BorrowedLeaseRegistry`/`NoCopyLeaseImporter`、per-submission retain 与 destroying-drop 退休），`13b8516` 让 native provider 支持 staged lease 导入（同一 `LeaseRegistry` 与 `BufferResolver`），`79c9706` 让 `metal-api-ipc` 用 `memfd_create`/`shm_open` 与 `SCM_RIGHTS` 在两进程间导入同一 owner 映射，`7660f20` 让 native provider 用 `newBufferWithBytesNoCopy:` 映射整段页对齐 borrowed reservation 并按 view offset 绑定与写回，`0ed4f60` 在 `metal-api-ipc` 增加 owner→provider 的 `MCC1` 请求/响应命令通道、`RemoteProvider` 与 `serve_provider`（provider 侧重新 admit 后执行），`cd5fcf0` 让命令通道承载 `ComputeProvider::health` 与 `ImportStagedLease`/`ReleaseStagedLease`，`134d746` 让命令通道用 `SCM_RIGHTS` 传递 owner 映射 descriptor（`ImportBorrowedLease`/`ReleaseBorrowedLease`，server 校验映射覆盖 reservation 并在 provider 释放前保持映射存活），`630260f` 让命令通道对超过单帧上限的请求/响应自动分块重组（默认 64 MiB 帧上限、256 MiB 上限，拒绝缺口/总数不符/交错帧），`574aca1` 修复命令通道在拒绝重复 borrowed-lease import 时未先消费 `SCM_RIGHTS` descriptor 导致后续帧失步的问题（先 drain descriptor 再返回 `lease_already_imported`，并加 import→重复拒绝→release→重新 import 的回归测试），`086b94c` 把非 `None` 的 `BufferView::attribute_stride` 从通用 `trace_contract_invalid` 提升为结构化 `Capability` 拒绝（slug `buffer_attribute_stride_unsupported`，含 core 测试与 IPC wire round-trip），`ce0296b` 把 `contract_error_refusal` 改为穷尽匹配（schema version 为 `trace_schema_unsupported` capability，pipeline/completion epoch mismatch 分别归入 resource 类与 `completion_epoch_mismatch`，writeback/completion 发布错误不再落到通用 args slug），`e891102` 修复 Windows GNU 构建（`descriptor_error` 无条件化）并把 Unix socket IPC/命令通道/共享内存 smoke 用例按 `#[cfg(unix)]` 门控，`3a073b5` 新增每次 push 构建/clippy Windows rail 的 `windows-cross-check` CI job，`89f1119` 记录 reims 适配器在 Windows RTX 5060 的真实运行，`afbbc1c` 让 `windows-cross-check` 同时准备 pinned reims 源码并 check Windows `reims-smoke`，`6cf23af` 给 `run-smoke.ps1` 增加 `-ProviderSmoke` 与 `-CaptureMatrix` 开关，`dd4db6a` 把 capture 矩阵写入时间戳目录以避免 `provider-capture` 拒绝覆盖，`8439f7e` 增加跨平台命名映射（Unix `shm_open`、Windows `CreateFileMappingW`/`OpenFileMappingW`）、命令帧后的 name/length 握手、`serve_provider_named` 与 TCP 命令/完成传输，并在 Lavapipe 与 Windows RTX 5060 两进程验证 no-copy 原地写入与重复 import 分帧，`c22ffca` 记录该验证与 CI/证据路径，`17ce2c9` 增加模拟设备丢失生命周期的 provider-smoke 回归（注入 hook + health/refusal/recreate 断言），`4cac064` 把 `health()` 暴露到对象 API 并让生命周期用例断言 lost/recovered 状态，`1accb7a` 增加对象 API 队列顺序回归（两个 command buffer 链式依赖、commit 顺序与精确写回），`1a630c2` 把关闭的命令通道归类为 `provider_unavailable`/`RetryAfterRecreate`（帧/契约错误仍为 `command_transport_failed`，未命名映射改为 args 类 `shared_mapping_unnamed`，新增单次握手后断连的 IPC 单测），`7acdbae` 增加真实 command 子进程被 kill 后的端到端断连用例，`2593794` 增加互不重叠 command buffer 同时在途的并行用例，`7b631f4` 让 Vulkan 设备侧把独立异步提交按 round-robin 分发到最多 4 个 device queue（Lavapipe `queues=1 distributed=false`，RTX 5060 `queues=4 distributed=true`）；`2fda388` 把全局 execution lock 改为 per-queue host lock、按“最少在途 + 轮询打破平局”选队列，并新增单 command buffer 内依赖与 probe-backed 多队列并发 enqueue 用例；`4181b08` 在专用 compute-only family 再创建最多 4 个 queue（RTX 5060 真机 `queues=8 families=2 distinct=8`，Lavapipe `queues=1 families=1`）；`cc38dd4` 新增 v9 command buffer 边界 conformance 套件（复用 v7 的三个 `subset_chain_*` fixture，把 dispatch 序列拆到 2–4 个顺序提交的 command buffer，跨边界均为真实数据依赖；Rust capture 与 Swift oracle 运行前校验分区、在提交/等待边界重新快照 landed bytes，报告为合并后的最终 per-view landing）。
...
- completion 驱动的 lease（`d26359c`）：`metal-api-core::provider` 新增 `LeaseLedger`、`LeaseObservation` 与 `disposition_retires_resources`；owner 注册 `LeaseReservation` 并绑定 submission token，只有全部 token 退休后才可释放，`device_lost` 作为 teardown guarantee；6 个新单测通过。provider 侧 lease import 仍未实现。CI run `34226648778` 四 job 全绿（165 Rust + 115 Python，五路 v1–v8 一致），证据在 `evidence/lease-ledger-d26359c-2026-09-08/run-34226648778/`。
- 跨进程 completion 镜像（`c610cab`）：`metal-api-core::completion::wire` 新增 `CompletionMessage`、`CompletionTokenUpdate`、`CompletionDeviceUpdate`、`CompletionSequence`、`CompletionFailure` 与 `CompletionMirror`。每个 token 与设备 health 流都有单调 sequence，重复/乱序/合并通知可安全重放；首个终态胜出，冲突终态取更小 sequence；`Exhausted` 只拒绝新工作而保留 in-flight 观察，`DeviceLost` 把非终态 token 标记为 `DeviceLost` 并作为 teardown 证据；`TimedOut` 是 owner 本地决定不上 wire，`CompletedVisible` 只表示可通过数据通道取回结果；`CompletionMirror::observe_into` 把收敛观察应用到 `LeaseLedger`。12 个新单测通过。provider 侧 publisher 与实际 transport 仍未实现。CI run `34227545361` 四 job 全绿（177 Rust + 115 Python，五路 v1–v8 一致），证据在 `evidence/completion-wire-c610cab-2026-09-08/run-34227545361/`。
- completion 发送端与 loopback transport（`9fd56eb`）：`metal-api-core::completion::wire` 新增 `CompletionPublisher`（`CompletionMirror` 的发送端对偶，按 token/device-health 流分配单调 sequence，终态幂等重放，拒绝冲突终态、终态后的非终态和设备丢失后的非终态更新）与 `LoopbackTransport`（保序内存测试/诊断 transport，可反序投递）；`ContractError` 增加 `CompletionPublishAfterTerminal`/`CompletionPublishAfterDeviceLost`/`CompletionHealthRegression`。9 个新单测通过。实际 IPC transport 与 provider 接线仍未实现。CI run `34228288218` 四 job 全绿（186 Rust + 115 Python，五路 v1–v8 一致），证据在 `evidence/completion-publisher-9fd56eb-2026-09-08/run-34228288218/`。
- completion IPC transport（`2324252`）：新增 `metal-api-ipc` workspace crate。`codec` 是版本化 `MCW1` 帧（magic + big-endian u32 长度，1 MiB payload 上限、4096 字节 failure slug 上限，解码校验 tag/枚举/UTF-8/尾随字节并调用 `CompletionMessage::validate`）；`transport::CompletionTransport<R, W>` 每次 send/recv 恰好一帧并带 sent/received 计数、flush 与半部访问；Unix 目标提供 `pair`/`connect`/`from_stream`、读写超时与 `UnixListenerTransport`；`completion_echo` 助手二进制支撑真实子进程 stdin/stdout 往返测试。13 个新测试（9 单测 + 4 集成）通过。provider 接线与 owner 侧 receive/lease 集成仍未实现。CI run `34229075904` 四 job 全绿（199 Rust + 115 Python，五路 v1–v8 一致），证据在 `evidence/ipc-transport-2324252-2026-09-08/run-34229075904/`。
- owner 侧 completion 接收端（`3d4dbd7`）：`metal-api-ipc::receiver` 新增 `CompletionReceiver<R, W>`，组合 `CompletionTransport` 与 `CompletionMirror`：`recv` 读一帧、应用到镜像并返回 `MirrorOutcome::{Applied, Ignored}`（分别计数）；`EndpointError` 区分 codec 错误与 contract 错误；`observe_into` 把镜像收敛观察喂给 `LeaseLedger`（完成退休 lease、设备丢失释放全部）。5 个集成测试覆盖 publisher→socket→mirror→lease 退休、终态重放忽略、设备丢失释放、epoch 不匹配拒绝与真实子进程 echo 往返。真实 provider 仍未接线。CI run `34229520518` 四 job 全绿（207 Rust + 115 Python，五路 v1–v8 一致），证据在 `evidence/receiver-3d4dbd7-2026-09-08/run-34229520518/`。
- provider 侧 outbox 与 IPC writer（`9226ef2`/`a06f7dc`/`95bedff`/`83b3f23`）：`CompletionRecord` 增加首个终态观察者（释放 record 锁后回调一次）；`CompletionOutbox` 把 `CompletionPublisher` 绑定到 `CompletionSink`，发布 admission/终态/设备 health 并记录拒绝；Vulkan 与 native provider 通过 epoch 校验的 `with_completion_outbox` 接入（未接时行为不变），native 的 health 发布改为读取已持有的 state 避免锁重入；`metal-api-ipc::sender::spawn_writer` 提供后台写线程，`deliver` 不阻塞 I/O 且逐帧 flush。10 个新 core 测试 + 2 个 sender 单测 + 2 个端到端集成测试通过；provider-smoke 取消用例断言 outbox 流 `Submitted, Cancelled, Submitted, CompletedVisible`。CI run `34230881817` 四 job 全绿（221 Rust + 115 Python，五路 v1–v8 一致，macOS 含 native outbox 断言），证据在 `evidence/completion-outbox-83b3f23-2026-09-08/run-34230881817/`。
- provider 经 IPC 到 owner 的端到端冒烟（`8913e37`/`6500b94`）：`provider-smoke` 新增 `run_completion_ipc`——真实 standalone Vulkan async provider 的 outbox 经 `metal-api-ipc::sender::spawn_writer` 写到 Unix socket pair，owner 侧 `CompletionReceiver` 读帧进镜像并 `observe_into(LeaseLedger)`，断言 `provider_completion_ipc transport=unix outbox=Submitted,CompletedVisible lease=retired`；`6500b94` 补记 `integration/reims/Cargo.lock` 的 `metal-api-ipc` 条目（`8913e37` 首次 run `34231349338` 因 lock 缺条目在 reims-integration 失败）。CI run `34231623347` 四 job 全绿（221 Rust + 115 Python，五路 v1–v8 一致，macOS 137 core + 10 native，reims 3 tests + Lavapipe reims smoke），证据在 `evidence/completion-ipc-6500b94-2026-09-08/run-34231623347/`。
- 两进程 owner/provider 拆分（`9ac0dfe`）：`provider-smoke` 增加 `--completion-child <socket>` 模式——子进程自建 Vulkan executor 与 async provider，连到 owner 的 Unix listener，经 `CompletionOutbox` + `metal-api-ipc::sender::spawn_writer` 发布 admission 与 `CompletedVisible`，并先打印 `handshake epoch=<n> submission=<n>`；父进程作为 owner 只持有 listener、`CompletionReceiver`、镜像与 `LeaseLedger`，从握手学到 epoch/token 后把终态观察退休为 lease 释放，并要求子进程成功退出。README、`docs/PROVIDER-B1.md` 与 `docs/SHARED-PROVIDERS.md` 同步该覆盖并移除“IPC transport 尚未落地”的措辞。CI run `34232217147` 四 job 全绿（221 Rust + 115 Python，五路 v1–v8 一致，macOS 137 core + 10 native，reims 3 tests + Lavapipe reims smoke），证据在 `evidence/completion-ipc-process-9ac0dfe-2026-09-08/run-34232217147/`。
- Vulkan staged lease import（`ab97ac9`）：`metal_api_core::provider` 增加 `StagedLease`、`LeaseRegistry` 与 `LeaseImporter`——registry 校验 reservation 窗口字节长度、拒绝重复/未知 lease、按 view 从窗口切片，并把 staged reservation 与 admitted `ResourceTableSnapshot`、设备 epoch 交叉校验；Vulkan provider 声明 `StorageMode::StagedLease`、内嵌 registry、在 submit 时解析 staged view，`BorrowedNoCopy` 仍 typed-refuse。provider-smoke 导入 8 字节窗口、执行从中切出的 4 字节 view、校验写回、经 owner `LeaseLedger` 退休 lease，release 后再提交被 `lease_not_imported` 拒绝。CI run `34233131601` 四 job 全绿（224 Rust + 115 Python，五路 v1–v8 一致，macOS 140 core + 10 native，reims 3 tests + Lavapipe reims smoke），证据在 `evidence/staged-lease-ab97ac9-2026-09-08/run-34233131601/`。
...
1. 异步提交与结果回收：契约、对象 API、Vulkan 与 native Metal provider 均已支持 `Submitted` → `wait` → `readback` → 写回（Vulkan 用设备侧 fence，native 用 `MTLCommandBuffer` handler）、共享可配置的 observation deadline（默认 20 秒）、显式取消（`CompletionDisposition::Cancelled` + `ComputeProvider::cancel` + `CommandBuffer::cancel`）以及超时后的资源回收（Vulkan retirement 线程 / native completion handler）；`a0b10e1` 已加入 provider 级 `AbandonmentBudget`、`ProviderHealth` 与 Vulkan 设备丢失销毁路径，预算耗尽后 fail closed；`7e67a0e` 补齐 native `MTLCommandBufferError::DeviceRemoved` 分类与对象 API 提交期 `DeviceLost`/`Exhausted` 错误矩阵；`c610cab` 落地跨进程 completion 的值/顺序契约，`9fd56eb` 落地发送端 publisher/loopback，`2324252` 落地 `metal-api-ipc` 字节流 transport（codec + socket/子进程集成），`3d4dbd7` 落地 owner 侧 `CompletionReceiver`（transport→mirror→lease 退休），`9226ef2`/`a06f7dc`/`95bedff` 落地 provider 侧 outbox 与终态观察者，`83b3f23` 落地 IPC 后台 writer。`0ed4f60` 已完成生产级 owner→provider `MCC1` 命令通道（compile/submit/wait/readback/cancel/release），`cd5fcf0` 让它承载 `health` 与 staged lease 导入/释放；`134d746` 把 descriptor 传递纳入命令协议（`SCM_RIGHTS` 传递 owner 映射、server 校验映射覆盖 reservation 并在 provider 释放前保持存活、断连时对仍被 retain 的映射不 unmap），`630260f` 补上大于单帧上限的 chunked data channel（默认 64 MiB 帧上限、256 MiB 上限，拒绝缺口/总数不符/交错帧）；`574aca1` 修复拒绝重复 borrowed import 时未 drain descriptor 导致的通道失步，`086b94c` 把非 `None` 的 attribute stride 改为结构化 capability 拒绝；仍缺 guest memory 导入（dma-buf 为 Windows 非目标；`ab97ac9` 为 Vulkan staged 拷贝、`42974ac` 为 Vulkan borrowed no-copy host mapping、`79c9706` 为跨进程 shared-memory、`7660f20` 为 native no-copy 映射），；`e891102` 恢复 Windows rail 并在 RTX 5060 上验证 borrowed no-copy host-pointer 导入（`provider_borrowed_lease lease=98 alignment=4096 copy_in=live copy_out=in_place`）；`8439f7e` 补上 Windows 共享内存句柄传输（命名 section + `OpenFileMappingW`，命令帧后传 name/length，`serve_provider_named` + TCP 传输），并在 RTX 5060 两进程验证 owner 映射原地观察到 GPU 写入、重复 import 被拒后通道保持分帧；`17ce2c9` 为设备丢失生命周期补了模拟注入回归（health/refusal/recreate，`4cac064` 起对象 API 也可查询 health）；`1accb7a` 为对象 API 队列补了 commit 顺序与依赖回归（两个 command buffer 链式依赖、async 执行、精确写回）；`1a630c2` 把关闭的命令通道归类为 `provider_unavailable`/`RetryAfterRecreate` 并补了单测，`7acdbae` 用真实子进程 kill 验证同一契约，`2593794` 验证互不重叠的 command buffer 可同时在途，`7b631f4` 让 Vulkan 设备侧把独立异步提交分发到最多 4 个 device queue，`2fda388` 用 per-queue host lock 实现真正的并发 enqueue、按最少在途 + 轮询选队列，并补单 command buffer 内依赖与多队列并发用例，`4181b08` 进一步使用专用 compute-only family（RTX 5060 真机 `queues=8 families=2 distinct=8`）；仍缺真实设备丢失、按资源范围（而非整 allocation）的 hazard 跟踪、优先级/公平性与跨 family 负载感知，以及更完整的生产错误传播。
...
- `metal-api-emulator` 远端 `main` 已更新到 `4181b08`（含 pin 升级、异步 completion/readback 契约、Vulkan 与 native Metal 异步 provider、Vulkan 设备侧 fence 等待、共享可配置 observation deadline、reims 适配器桥接与 reims CI job、Vulkan 超时回收、显式取消、native 超时后保持可用、v8 binary AIR encoding 套件、有界放弃预算与 health、native device removal 分类与对象 API 提交期错误矩阵、transport 无关的跨进程 completion 镜像层与发送端 publisher，`metal-api-ipc` 字节流 transport 与 owner 侧 completion receiver，以及 provider 侧 completion outbox/终态观察者与 IPC 后台 writer）；本地与远端同步。`76f6102` 的 CI run `34216817216` 成功；`31a0ba8` 的 CI run `34218661059` 成功；`ebca75e` 的 CI run `34219297172` 成功；`15bf79a` 的 CI run `34219887881` 成功；`45a7427` 的 CI run `34220240822` 成功；`6e5df5b` 的 CI run `34220576077` 成功；`81d6cbe` 的 CI run `34220849337` 成功；`eed4a53` 的 CI run `34221388956` 成功；`909a5fb` 的 CI run `34221535473` 成功；`0630022` 的 CI run `34221811367` 成功；`008fe7d` 的 CI run `34222084119` 成功；`3bf3c04` 的 CI run `34223201314` 在 native-oracle-build 失败（oracle 拒绝 v8，standalone/reims 通过），由 `d8e42bf` 修复；`d8e42bf` 的 CI run `34223294821` 成功（四 job 全绿，五路 v1–v8 共 29 cases 一致）；`042acc8` 的 CI run `34223676120` 成功（四 job 全绿）；`a0b10e1` 的 CI run `34224461611` 成功（四 job 全绿，156 Rust + 115 Python，五路 v1–v8 一致）；`7e67a0e` 的 CI run `34225380992` 成功（四 job 全绿，159 Rust + 115 Python，五路 v1–v8 一致）；`8d2a4ca` 的 CI run `34226091972` 成功（四 job 全绿）；`d26359c` 的 CI run `34226648778` 成功（四 job 全绿，165 Rust + 115 Python，五路 v1–v8 一致）；`c610cab` 的 CI run `34227545361` 成功（四 job 全绿，177 Rust + 115 Python，五路 v1–v8 一致）；`9fd56eb` 的 CI run `34228288218` 成功（四 job 全绿，186 Rust + 115 Python，五路 v1–v8 一致）；`2324252` 的 CI run `34229075904` 成功（四 job 全绿，199 Rust + 115 Python，五路 v1–v8 一致）；`3d4dbd7` 的 CI run `34229520518` 成功（四 job 全绿，207 Rust + 115 Python，五路 v1–v8 一致）；`83b3f23` 的 CI run `34230881817` 成功（四 job 全绿，221 Rust + 115 Python，五路 v1–v8 一致）；`8913e37` 的 CI run `34231349338` 在 reims-integration 失败（lock 缺 `metal-api-ipc`），由 `6500b94` 修复；`6500b94` 的 CI run `34231623347` 成功（四 job 全绿，221 Rust + 115 Python，五路 v1–v8 一致，macOS 137 core + 10 native，reims 3 tests + Lavapipe reims smoke）；`9ac0dfe` 的 CI run `34232217147` 成功（四 job 全绿，221 Rust + 115 Python，五路 v1–v8 一致，macOS 137 core + 10 native，reims 3 tests + Lavapipe reims smoke）；`ab97ac9` 的 CI run `34233131601` 成功（四 job 全绿，224 Rust + 115 Python，五路 v1–v8 一致，macOS 140 core + 10 native，reims 3 tests + Lavapipe reims smoke）。`42974ac` 的 CI run `34234349561` 成功（四 job 全绿，226 Rust + 115 Python，五路 v1–v8 一致，macOS 142 core + 10 native，reims 3 tests + Lavapipe reims smoke；provider-smoke 新增 borrowed no-copy case）。`13b8516` 的 CI run `34234725350` 成功（四 job 全绿，226 Rust + 115 Python，五路 v1–v8 一致，macOS 142 core + 11 native，reims 3 tests + Lavapipe reims smoke；新增 native staged lease 测试）。证据归档在 `evidence/conformance-v8-d8e42bf-2026-09-08/run-34223294821/`、`evidence/provider-v8-docs-042acc8-2026-09-08/run-34223676120/`、`evidence/provider-abandonment-a0b10e1-2026-09-08/run-34224461611/`、`evidence/device-removal-7e67a0e-2026-09-08/run-34225380992/`、`evidence/windows-v8-docs-8d2a4ca-2026-09-08/run-34226091972/`、`evidence/lease-ledger-d26359c-2026-09-08/run-34226648778/` 、`evidence/completion-wire-c610cab-2026-09-08/run-34227545361/`、`evidence/completion-publisher-9fd56eb-2026-09-08/run-34228288218/` 、`evidence/ipc-transport-2324252-2026-09-08/run-34229075904/` 、`evidence/receiver-3d4dbd7-2026-09-08/run-34229520518/` 、`evidence/completion-outbox-83b3f23-2026-09-08/run-34230881817/` 、`evidence/completion-ipc-6500b94-2026-09-08/run-34231623347/` 、`evidence/completion-ipc-process-9ac0dfe-2026-09-08/run-34232217147/` 与 `evidence/staged-lease-ab97ac9-2026-09-08/run-34233131601/`。`42974ac` 的证据归档在 `evidence/borrowed-lease-42974ac-2026-09-08/run-34234349561/`，`13b8516` 的证据归档在 `evidence/native-staged-lease-13b8516-2026-09-08/run-34234725350/`。`79c9706` 的 CI run `34235576029` 成功（四 job 全绿，229 Rust + 115 Python，五路 v1–v8 一致，macOS 142 core + 11 native，reims 3 tests + Lavapipe reims smoke；provider-smoke 新增跨进程 shared-memory borrowed lease case）；`7660f20` 的 CI run `34236061739` 成功（四 job 全绿，229 Rust + 115 Python，五路 v1–v8 一致，macOS 142 core + 12 native，reims 3 tests + Lavapipe reims smoke；macOS 新增 native no-copy borrowed reservation 测试）；`0ed4f60` 的 CI run `34236490091` 成功（四 job 全绿，231 Rust + 115 Python，五路 v1–v8 一致，macOS 142 core + 12 native，reims 3 tests + Lavapipe reims smoke；provider-smoke 新增 owner→provider 命令通道 case）；`cd5fcf0` 的 CI run `34236828326` 成功（四 job 全绿，231 Rust + 115 Python，五路 v1–v8 一致，macOS 142 core + 12 native，reims 3 tests + Lavapipe reims smoke；命令通道新增 health 与 staged lease 导入/释放）。证据归档在 `evidence/ipc-owner-mappings-79c9706-2026-09-08/run-34235576029/`、`evidence/native-no-copy-7660f20-2026-09-08/run-34236061739/`、`evidence/provider-command-0ed4f60-2026-09-08/run-34236490091/` 与 `evidence/provider-command-lease-cd5fcf0-2026-09-08/run-34236828326/`。`134d746` 的 CI run `34237784015` 成功（四 job 全绿，232 Rust + 115 Python，五路 v1–v8 一致，macOS 142 core + 12 native，reims 3 tests + Lavapipe reims smoke；provider-smoke 新增命令通道 descriptor-backed borrowed lease case）；`630260f` 的 CI run `34237987177` 成功（四 job 全绿，234 Rust + 115 Python，五路 v1–v8 一致，macOS 142 core + 12 native，reims 3 tests + Lavapipe reims smoke；provider-smoke 把两侧帧上限降到 1 KiB，所有请求与 readback 响应全程走 chunk 路径）。证据归档在 `evidence/command-descriptors-134d746-2026-09-08/run-34237784015/` 与 `evidence/command-chunks-630260f-2026-09-08/run-34237987177/`。`574aca1` 的 CI run `34238803573` 成功（四 job 全绿，235 Rust + 115 Python，五路 v1–v8 一致，macOS 142 core + 12 native，reims 3 tests + Lavapipe reims smoke；provider-smoke 新增重复 borrowed import 被拒后通道保持同步的回归 case）；`086b94c` 的 CI run `34239161666` 成功（四 job 全绿，236 Rust + 115 Python，五路 v1–v8 一致，macOS 143 core + 12 native，reims 3 tests + Lavapipe reims smoke；attribute stride 走结构化 capability 拒绝）。证据归档在 `evidence/ipc-descriptor-drain-574aca1-2026-09-08/run-34238803573/` 与 `evidence/provider-attribute-stride-086b94c-2026-09-08/run-34239161666/`。`ce0296b` 的 CI run `34239846254` 成功（四 job 全绿，238 Rust + 115 Python，五路 v1–v8 一致，macOS 145 core + 12 native，reims 3 tests + Lavapipe reims smoke；拒绝分类穷尽匹配）；`e891102` 的 CI run `34240100192` 成功（四 job 全绿，238 Rust + 115 Python，五路 v1–v8 一致，macOS 145 core + 12 native，reims 3 tests + Lavapipe reims smoke；Windows GNU 构建恢复）；`3a073b5` 的 CI run `34240353880` 成功（五个 job 全绿，新增 `windows-cross-check` 首次通过：MinGW-w64 + `x86_64-pc-windows-gnu` 构建与 clippy）；`89f1119` 的 CI run `34240660149` 成功（docs-only，沿用前序验证）；`afbbc1c` 的 CI run `34241105149` 成功（五个 job 全绿，`windows-cross-check` 的 `reims-smoke` Windows check 通过）；`6cf23af` 的 CI run `34241217697` 成功（五个 job 全绿，脚本开关不影响既有门禁）；`dd4db6a` 的 CI run `34241258427` 成功（五个 job 全绿，脚本时间戳化）；`8439f7e` 的 CI run `34243286265` 成功（五个 job 全绿，241 Rust + 115 Python，五路 v1–v8 一致，macOS 145 core + 12 native，reims 3 tests + Lavapipe reims smoke；provider-smoke 新增 `PASS provider_named_command_process ... mapping=named ... duplicate=refused chunked=1024`）；`c22ffca` 的 CI run `34243697606` 成功（docs-only，沿用前序验证）；`17ce2c9` 的 CI run `34245993037` 成功（五个 job 全绿，241 Rust + 115 Python，五路 v1–v8 一致，macOS 145 core + 12 native，reims 3 tests + Lavapipe reims smoke；provider-smoke 新增 `PASS provider_device_lifecycle ... recreated=true writeback=exact`）；`4cac064` 的 CI run `34246529497` 成功（五个 job 全绿，242 Rust + 115 Python，五路 v1–v8 一致，macOS 146 core + 12 native，reims 3 tests + Lavapipe reims smoke；对象 API `Device::health()` 暴露 lost/recovered 状态，用例输出新增 `object_health=exposed`）。`1accb7a` 的 CI run `34247006309` 成功（五个 job 全绿，242 Rust + 115 Python，五路 v1–v8 一致，macOS 146 core + 12 native，reims 3 tests + Lavapipe reims smoke；provider-smoke 新增 `PASS provider_object_queue_ordering ... ordering=commit_reservation writeback=exact`）。`1a630c2` 的 CI run `34247559667` 成功（五个 job 全绿，243 Rust + 115 Python，五路 v1–v8 一致，macOS 146 core + 12 native，reims 3 tests + Lavapipe reims smoke）；`7acdbae` 的 CI run `34247668653` 成功（五个 job 全绿，243 Rust + 115 Python，五路 v1–v8 一致，macOS 146 core + 12 native，reims 3 tests + Lavapipe reims smoke；provider-smoke 新增 `PASS provider_disconnect_process ... health=Exhausted refusal=provider_unavailable retry=RetryAfterRecreate killed=true`）；`2593794` 的 CI run `34247751574` 成功（五个 job 全绿，243 Rust + 115 Python，五路 v1–v8 一致，macOS 146 core + 12 native，reims 3 tests + Lavapipe reims smoke；provider-smoke 新增 `PASS provider_object_parallel_commands ... in_flight=2 writeback=exact`）。设备丢失生命周期真机证据在 `evidence/windows-lifecycle-17ce2c9-2026-09-08/` 与 `evidence/windows-lifecycle-4cac064-2026-09-08/`，CI 证据在 `evidence/windows-lifecycle-17ce2c9-2026-09-08/run-34245993037/` 与 `evidence/windows-lifecycle-4cac064-2026-09-08/run-34246529497/`。Windows 命名映射真机证据在 `evidence/windows-named-8439f7e-2026-09-08/`，CI 证据在 `evidence/windows-named-8439f7e-2026-09-08/run-34243286265/`。证据归档在 `evidence/windows-rail-afbbc1c-2026-09-08/run-34241105149/`、`evidence/windows-rail-6cf23af-2026-09-08/run-34241217697/` 与 `evidence/windows-rail-dd4db6a-2026-09-08/run-34241258427/`。其余证据归档在 `evidence/provider-refusal-classes-ce0296b-2026-09-08/run-34239846254/`、`evidence/windows-rail-e891102-2026-09-08/run-34240100192/` 与 `evidence/windows-ci-3a073b5-2026-09-08/run-34240353880/`；Windows 真机证据在 `evidence/windows-rtx5060-e891102-2026-09-08/`（provider-smoke borrowed no-copy + reims-smoke + 24 个 v1–v8 捕获）；Windows 队列真机证据在 `evidence/windows-queue-1accb7a-2026-09-08/`，CI 证据在 `evidence/windows-queue-1accb7a-2026-09-08/run-34247006309/`；命令通道断连 CI 证据在 `evidence/provider-channel-close-1a630c2-2026-09-08/run-34247559667/` 与 `evidence/provider-disconnect-7acdbae-2026-09-08/run-34247668653/`；并行命令真机证据在 `evidence/provider-parallel-2593794-2026-09-08/`，CI 证据在 `evidence/provider-parallel-2593794-2026-09-08/run-34247751574/`。多 queue 真机证据在 `evidence/vulkan-multiqueue-7b631f4-2026-09-09/`（`queues=4 distributed=true`），CI 证据在 `evidence/vulkan-multiqueue-7b631f4-2026-09-09/run-34248584716/`。并发 enqueue 真机证据在 `evidence/provider-concurrent-enqueue-2fda388-2026-09-09/`（`queue_a=0 queue_b=1`），CI 证据在 `evidence/provider-concurrent-enqueue-2fda388-2026-09-09/run-34249125084/`（attempt 2；attempt 1 仅 artifact 上传 403）；专用 compute family 真机证据在 `evidence/compute-family-queues-4181b08-2026-09-09/`（`queues=8 families=2 distinct=8`），CI 证据在 `evidence/compute-family-queues-4181b08-2026-09-09/run-34249663615/`。
- `qemu-reims-vgpu`：master 仍为 `300438f`（2026-07-24），没有与 provider 直接相关的新 master 变化。

### 13.5 下一步

- 有界回收：`a0b10e1`/`7e67a0e`/`d26359c` 已完成 provider 级预算、`ProviderHealth`、Vulkan 设备丢失销毁路径、native `MTLCommandBufferError::DeviceRemoved` 分类、对象 API 的 `DeviceLost`/`Exhausted` 端到端断言与 core `LeaseLedger`；跨进程 completion 的值/顺序契约（`c610cab`）、发送端 publisher/loopback transport（`9fd56eb`）与字节流 IPC transport（`2324252`）、owner 侧 completion receiver/lease 接入（`3d4dbd7`）、provider 侧 outbox/终态观察者（`9226ef2`/`a06f7dc`/`95bedff`）与 IPC 后台 writer（`83b3f23`）、真实 provider→IPC→owner 端到端冒烟（`8913e37`）与两进程 owner/provider 拆分（`9ac0dfe`）、Vulkan staged lease import（`ab97ac9`）、Vulkan borrowed no-copy host mapping 导入（`42974ac`）、native staged lease 导入（`13b8516`）、跨进程 shared-memory owner 映射导入（`79c9706`）、native no-copy borrowed reservation 映射（`7660f20`）、owner→provider `MCC1` 命令通道（`0ed4f60`）、命令通道 health/staged lease（`cd5fcf0`）、命令通道 descriptor 传递（`134d746`）与 chunked data channel（`630260f`）、拒绝重复 import 的 descriptor drain（`574aca1`）与 attribute stride 结构化 capability 拒绝（`086b94c`）、穷尽拒绝分类（`ce0296b`）、Windows rail 恢复（`e891102`）与 Windows CI job（`3a073b5`，`afbbc1c` 起同时交叉检查 Windows reims 适配器，`6cf23af`/`dd4db6a` 给 `run-smoke.ps1` 增加 provider/capture 开关并时间戳化输出）已完成；dma-buf 是 Windows 非目标（无内核对象等价物，保底是 host-pointer 导入与拷贝 rail），host-pointer 导入已在 Windows RTX 5060 单进程路径验证（`copy_out=in_place`），Windows 共享内存句柄传输已由 `8439f7e` 完成（命名 section + TCP 命令通道，Lavapipe 与 RTX 5060 两进程验证）；`17ce2c9` 为 Vulkan 设备丢失生命周期补了模拟注入回归（health/refusal/recreate，`4cac064` 起对象 API 也可查询 health）；`1accb7a` 为对象 API 队列补了 commit 顺序/依赖回归（两个 command buffer 链式依赖、async 执行、精确写回），`1a630c2`/`7acdbae` 把关闭的命令通道归类为 `provider_unavailable`/`RetryAfterRecreate` 并用单测与真实子进程 kill 验证，`2593794` 验证互不重叠命令可同时在途，`7b631f4`/`2fda388`/`4181b08` 完成设备侧多 queue 分发、per-queue host lock 并发 enqueue、最少在途调度与专用 compute-only family（RTX 5060 真机 `queues=8 families=2 distinct=8`），下一步做真实设备丢失、资源范围 hazard 跟踪、优先级/公平性与跨 family 负载感知，以及更完整的错误传播，以及 guest memory、图形/显示与 reims Gate 2/3。
- v8 conformance 已完成：`3bf3c04`/`d8e42bf`/`042acc8` 增加 binary AIR encoding 维度（raw/wrapped），并保持五路 v1–v8（29 cases）全绿；下一步评估新的语义维度（候选：更复杂的 buffer 布局/offset、多 command buffer 提交或错误传播路径），仍不宣称完整 Metal conformance；候选 v9 维度 `BufferView.attribute_stride` 暂缓：执行它需要 metal2vulkan 暴露 dynamic compute stageInputDescriptor reflection，pin `43c46ac` 不提供；当前所有 suite 均为 `None`，非 None 值走结构化 capability 拒绝。
...
### 13.10 2026-09-15 增量：未定义初值（v20 `LoadOp::DontCare`）

- **契约**（`e4a3563`）：三种 load op 全部放行；`UnsupportedAttachmentLoadOp` 变体保留但不再产生。
- **两条 rail**（`a04544a`/`70dd974`）：Vulkan `UNDEFINED` + `LOAD_OP_DONT_CARE`、不解析/不上传初值、
  不要求 `TRANSFER_DST`（`OffscreenColorAttachment.clear` 改为 `load: LoadOp` 单一事实源）；
  native `MTLLoadAction::DontCare`、不预置 texture、`DontCare + Some(bytes)` 以
  `render_attachment_initial_mismatch` 拒绝。
- **对象 API**（`3ff66dc`）：`RenderAttachmentLoad::DontCare`。
- **观测通道**（`a4fd148`）：`load: "dontcare"` 不得带 `clear_hex`/`initial_hex`，期望逐 texel 相同，
  且必须与 declaring 字节不同；v20 fixture `dontcare_load_quad_2x2` 期望 `4080c0ff`×4。
- **证据**：CI run `34925106004` 五 job 全绿（main `dccac6d`），五轨 parity 到 `compute-buffer-v20`，
  Apple 五自检全 PASS（`evidence/conformance-v20-dccac6d-2026-09-15/`）；RTX 5060 双轨
  （`evidence/windows-rtx5060-v20-dccac6d-2026-09-15/`）；本地 `GATES_OK` +
  `LAVAPIPE_SMOKE_OK suites=20 captures=60`。
- 至此 load/store 矩阵（`Clear/Load/DontCare` × `Store/DontCare`）全部放行；仍未做：3/4 附件、
  非双 `rgba8_unorm` 的格式组合、深度/模板、实例化步进、动态状态、heap aliasing、
  真实设备丢失恢复、guest memory 的 reims 侧接线与 Gate 2/3。

### 13.11 2026-09-15 增量：附件格式矩阵（v21 `bgra8_unorm`）

- **契约与 rail 零改动**：`Bgra8Unorm` 早已在 `AttachmentFormat::ADMITTED` 与两条 rail 的映射里；
  本增量把它的**观测**补齐（比较器白名单、capture 的逐附件格式注册、Swift 的
  `MTLPixelFormat.bgra8Unorm`）。
- **语义**：同一个 float4 store 在 B8G8R8A8 布局里读回 `c08040ff`（B,G,R,A）；
  `clear_hex` 仍是"附件内存里的字节"。
- **fixture**（`6b50c5f`）：`bgra8_clear_2x2`，期望 `c08040ff`×4，计数 2/2；
  `test_suite_v13.py` 的未准入探针改为 `r32float`；shipped-plan 计数 21。
- **证据**：CI run `34989832299` 五 job 全绿（main `f821733`），五轨 parity 到
