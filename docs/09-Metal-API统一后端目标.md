...
- `metal-api-emulator` 的 `shared-provider-objects` 分支（推送目标 `origin/main`）：HEAD `31a0ba8`（2026-09-08），已推送；其中 `69dab48` 为 metal2vulkan pin 升级，`76f6102` 为异步 completion/readback 契约，`31a0ba8` 为 Vulkan 真实异步 provider。
...
- 验证（`31a0ba8` 本地）：86 core / 7 native / 37 Vulkan / 14 capture = 144 Rust + 113 Python 全通过；Lavapipe standalone/provider smoke、v1–v7 同步 direct/object 捕获、v1–v7 异步 object 捕获（共 26 case）全部通过；fmt/clippy/rustdoc 通过。native provider 仍同步返回 `CompletedVisible`。
...
1. 异步提交与结果回收：契约、对象 API 与 Vulkan provider 已支持 `Submitted` → `wait` → `readback` → 写回；Vulkan 异步模式用 worker 桥接现有同步 executor，仍缺拆分 fence 等待的真正设备侧异步、native Metal completion handler、取消、deadline、超时资源回收、completion 驱动的 lease 生命周期，以及生产错误传播。
...
- `metal-api-emulator` 远端 `main` 已更新到 `31a0ba8`（含 pin 升级、异步 completion/readback 契约与 Vulkan 异步 provider）；本地与远端同步。`76f6102` 的 CI run `34216817216` 成功；`31a0ba8` 的 CI run `34218661059` 成功：standalone（144 Rust + 113 Python + Lavapipe v1–v7 direct/object/async-object）、macOS native-oracle、五路 compare（v1–v7 共 26 cases）全绿。证据归档在 `evidence/vulkan-async-31a0ba8-2026-09-08/run-34218661059/`。
- `qemu-reims-vgpu`：master 仍为 `300438f`（2026-07-24），没有与 provider 直接相关的新 master 变化。

### 13.5 下一步

- 在 native provider 上实现 `Submitted` + `readback`（`MTLCommandBuffer` completion handler）；Vulkan 侧进一步拆分 fence 等待，去掉 per-submission worker。
- 设计取消与超时资源回收：`TimedOut` 重试已有，但缺取消 token、deadline、设备丢失后的完成记录清理，以及 completion 驱动的 lease 释放。
