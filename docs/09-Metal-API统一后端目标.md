...
4. reims 生产接入：只有可选、离线的 Vulkan A/B executor（pin 升级后适配器已在 `6e5df5b` 修复，并由 `81d6cbe` 的 CI job 持续验证）；生产 guest/display 路径未调用 canonical Metal provider；缺真实多进程 provider↔owner 拆分（provider outbox、IPC writer 与 owner receiver 已由 `9226ef2`–`83b3f23` 提供，并由 `8913e37` 单进程端到端打通）、真实设备生命周期、生产队列调度和错误传播；Gate 2 未通过。
