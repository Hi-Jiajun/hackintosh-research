...
- `metal-api-emulator` 的 `shared-provider-objects` 分支（推送目标 `origin/main`）：HEAD `76f6102`（2026-09-08），已推送；其中 `69dab48` 为 metal2vulkan pin 升级，`76f6102` 为异步 completion/readback 契约。
...
- 验证（`76f6102` 本地）：86 core / 7 native / 34 Vulkan / 14 capture = 141 Rust + 113 Python 全通过；Lavapipe standalone/provider smoke 与 v1–v7 共 26 个 object case 通过；fmt/clippy/rustdoc 通过。Vulkan/native provider 仍同步返回 `CompletedVisible`，因此五路径 capture 行为不变；异步路径当前由契约 fake 覆盖。
...
1. 异步提交与结果回收：契约与对象 API 已支持 `Submitted` → `wait` → `readback` → 写回，buffer reservation 跨 completion；仍缺真正的异步 provider 实现（Vulkan/native 仍同步返回 `CompletedVisible`）、fence/event 或 completion handler 驱动的后台完成、取消、超时资源回收、completion 驱动的 lease 生命周期，以及生产错误传播。
...
- `metal-api-emulator` 远端 `main` 已更新到 `76f6102`（含 `69dab48` pin 升级与异步 completion/readback 契约）；本地与远端同步。CI run `34216817216` 成功：standalone（141 Rust + 113 Python + Lavapipe v1–v7 direct/object）、macOS native-oracle、五路 compare（v1–v7 共 26 cases）全绿。证据归档在 `evidence/async-completion-readback-76f6102-2026-09-08/run-34216817216/`。
- `qemu-reims-vgpu`：master 仍为 `300438f`（2026-07-24），没有与 provider 直接相关的新 master 变化。

### 13.5 下一步

- 在真实 provider 上实现 `Submitted` + `readback`：Vulkan 侧用 fence/event 或 completion handler 延迟提交结果，native 侧用 `MTLCommandBuffer` completion handler；这是生产接线的主要阻塞项。
