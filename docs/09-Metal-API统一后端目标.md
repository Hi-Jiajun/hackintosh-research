...
- `metal-api-emulator` 的 `shared-provider-objects` 分支（推送目标 `origin/main`）：HEAD `6e5df5b`（2026-09-08），已推送；其中 `69dab48` 为 metal2vulkan pin 升级，`76f6102` 为异步 completion/readback 契约，`31a0ba8` 为 Vulkan 真实异步 provider，`ebca75e` 为 native Metal completion handler 与共享 `CompletionRecord`，`15bf79a` 为 Vulkan 设备侧 fence 等待，`45a7427` 为共享可配置 observation deadline，`6e5df5b` 为 reims 适配器跨 translator 版本桥接。
...
4. reims 生产接入：只有可选、离线的 Vulkan A/B executor（pin 升级后适配器已在 `6e5df5b` 修复并重新验证）；生产 guest/display 路径未调用 canonical Metal provider；缺跨进程协议、真实设备生命周期、生产队列调度和错误传播；Gate 2 未通过。
...
- `metal-api-emulator` 远端 `main` 已更新到 `6e5df5b`（含 pin 升级、异步 completion/readback 契约、Vulkan 与 native Metal 异步 provider、Vulkan 设备侧 fence 等待、共享可配置 observation deadline 与 reims 适配器桥接）；本地与远端同步。`76f6102` 的 CI run `34216817216` 成功；`31a0ba8` 的 CI run `34218661059` 成功；`ebca75e` 的 CI run `34219297172` 成功；`15bf79a` 的 CI run `34219887881` 成功；`45a7427` 的 CI run `34220240822` 成功；`6e5df5b` 的 CI run `34220576077` 成功：standalone（147 Rust + 113 Python + Lavapipe v1–v7 direct/object/async-object）、macOS native-oracle、五路 compare（v1–v7 共 26 cases）全绿。证据归档在 `evidence/reims-bridge-6e5df5b-2026-09-08/`。
